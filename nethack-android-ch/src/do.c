...
polymorph_sink()
{
    ...
    pline_The("水槽变形为了 %s!", (levl[u.ux][u.uy].typ == THRONE)
                                              ? "一个王座"
                                              : surface(u.ux, u.uy));
    newsym(u.ux, u.uy);
}
...
register struct obj *obj;
{
    ...
    if (!Blind && !ideed && obj->otyp != RIN_HUNGER) {
        ...
        case RIN_FIRE_RESISTANCE:
            pline_The("热水龙头明亮地闪烁了片刻.");
            break;
        case RIN_COLD_RESISTANCE:
            pline_The("冷水龙头明亮地闪烁了片刻.");
            break;
