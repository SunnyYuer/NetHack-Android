...
register xchar ox, oy;
{
    ...
    pline("%s %s!", Monnam(shkp), !ANGRY(shkp) ? "生气了" : "火冒三丈");
    shkp->mx = sx, shkp->my = sy;
    hot_pursuit(shkp);
}

STATIC_VAR const char no_money[] = "此外, 你%s没有钱.";
...
proceed:
    ...
    if (!eshkp->billct && !eshkp->debit) {
        ...
        if (!ltmp && NOTANGRY(shkp)) {
            ...
            pline("%s 要你的命, 不是你的钱!", Monnam(shkp));
            if (umoney < 1000L) {
                if (!umoney)
                    pline(no_money, stashed_gold ? "似乎" : "");
                else
...
int croaked;
{
        ...
        if (cansee(shkp->mx, shkp->my) && croaked) {
            ...
            if (has_head(shkp->data) && !rn2(2))
                Sprintf(takes, ", 摇摇%s %s,", mhis(shkp),
                        mbodypart(shkp, HEAD));
            pline("%s %s看着你的尸体%s 然后%s.", Monnam(shkp),
    ...
    if (eshkp->following || ANGRY(shkp) || take) {
        ...
        if (loss > umoney || !loss || roomno == eshkp->shoproom) {
            ...
            money2mon(shkp, loss);
            context.botl = 1;
            pline("%s %s %ld %s %s欠%s.", Monnam(shkp), takes, loss,
                  currency(loss),
...
boolean ininv, dummy, silent;
{
    ...
    if (!muteshk(shkp) && !silent) {
        ...
        if (!ltmp) {
            pline("%s 对%s没兴趣.", Monnam(shkp), the(xname(obj)));
            return;
        }
        if (!ininv) {
...
boolean peaceful, silent;
{
    ...
    if (peaceful) {
        ...
        if (!silent) {
            ...
            if (obj->oclass == COIN_CLASS)
                You("%s欠 %s %ld %s!", still, mon_nam(shkp), value,
                    currency(value));
            else
                You("%s欠 %s %ld %s 对%s!", still, mon_nam(shkp), value,
                    currency(value), obj->quan > 1L ? "它们" : "它");
        }
    } else {
        ESHK(shkp)->robbed += value;
        ...
        if (!silent) {
            if (cansee(shkp->mx, shkp->my)) {
                Norep("%s 大声说: \" %s, 你是个小偷!\"", Monnam(shkp),
                      plname);
            } else
                Norep("你听见尖叫, \" 小偷!\"");
        }
...
register xchar x, y;
{
    ...
    if (shkp->mcanmove && !shkp->msleeping
        ...
        if (mnearto(shkp, x, y, TRUE) && !muteshk(shkp))
            verbalize("别挡我的路, 人渣!");
        if (cansee(x, y)) {
            pline("%s 敏捷地%s抓住了%s.", Monnam(shkp),
                  (x == shkp->mx && y == shkp->my) ? "" : "到达并",
                  the(xname(obj)));
            if (!canspotmon(shkp))
                map_invisible(x, y);
            delay_output();
